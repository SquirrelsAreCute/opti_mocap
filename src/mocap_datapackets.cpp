...
void MoCapDataFormat::parse()
{
  ...
  std::cout << "Marker Set Count : " << model.numMarkerSets << "\n";
  ...
  for (int i = 0; i < model.numMarkerSets; i++)
  {
	...
	std::cout << "Model Name: " << model.markerSets[i].name << "\n";

    // read number of markers that belong to the model
    read_and_seek(model.markerSets[i].numMarkers);
	std::cout << "Marker Set Count : " << model.markerSets[i].numMarkers << "\n";
  ...
  std::cout << "Rigid Body Count : " << model.numRigidBodies << "\n";
  ...
  for (int m = 0; m < model.numRigidBodies; m++)
  {
    ...
    read_and_seek(model.rigidBodies[m].pose.position);
    read_and_seek(model.rigidBodies[m].pose.orientation);
	std::cout << "ID : " << model.rigidBodies[m].ID << "\n";
	std::cout << "pos : " << model.rigidBodies[m].pose.position.x << " " << model.rigidBodies[m].pose.position.y << " " << model.rigidBodies[m].pose.position.z << "\n"; 
    ...
    /* /1* ROS_INFO("Number of rigid body markers: %d\n", model.rigidBodies[m].NumberOfMarkers); *1/ */
	/* if (model.rigidBodies[m].NumberOfMarkers > 20){ */
		/* /1* ROS_INFO("Too many markers!!!"); *1/ */
		/* model.rigidBodies[m].NumberOfMarkers = 0; /1* avoid huge values *1/ */
	/* } */
	...
	// skip unknown short (params since NatNet version 2.6 and later)
	seek(2);
